...
public class DeliveryServiceImpl extends AbsBaseService<DeliveryOrderEntity, DeliveryDao> implements DeliveryService {
            ...
            public Predicate toPredicate(Root<DeliveryOrderEntity> root, CriteriaQuery<?> query, CriteriaBuilder cb) {
                ...
                if(null!=search){
                    ...
                    if(StringUtils.isNotBlank(search.getStartTime())&&StringUtils.isNotBlank(search.getEndTime())){
                        where.add(cb.and(cb.between(createTime,
                                DateUtils.convertToDateTime(search.getStartTime()), DateUtils.convertToDateTime(search.getEndTime()))));
                    }
                }
    ...
    public void createDeliverOrder(DeliveryOrderEntity deliveryOrder) {
        ...
        try {
            ...
            boolean ans = QrCodeGenWrapper.of(content)
                    .setW(300)
        ...
        this.save(d);
        operateLogService.save(new OperateLogEntity("创建出库单id:"+d.getId(), OperateTypeEnum.CREATE,
                d.getId(), LogTypeEnum.DELIVERY,d.getCreateBy(), new Date()));
    }

    @Override
    @Transactional(rollbackFor = Exception.class)
    public void saveDelivery(DeliveryOrderEntity delivery, List<DeliveryStandardEntity> needDelete,
        ...
        operateLogService.save(new OperateLogEntity(logContent, type, delivery.getId(),
                LogTypeEnum.DELIVERY,operate, new Date()));
    }
}
